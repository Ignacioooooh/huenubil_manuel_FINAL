...
from proyecto_app.forms import FormAlumnos, FormInstituciones



def verautor(request):
    # Detalles del autor
    autor = {
        'nombre': 'Manuel',
        'apellido': 'Huenubil'
...
def agregar_instituciones(request):
    if request.method == 'POST':
        form = FormInstituciones(request.POST)
        if form.is_valid():
            form.save()
            return HttpResponseRedirect('/listar')  # Redirecciona a la vista de listado después de guardar
    else:
...
def agregar_inscripcion(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('listar')  # Redirecciona a la vista de listado después de guardar
    else:
        form = FormAlumnos()
